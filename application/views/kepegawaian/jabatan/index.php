                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" id="search" placeholder="Cari Jabatan" onkeyup="get_data()">
                            </div>
                        </div>
                    </div>
                </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="main_account_name" style="font-weight: 600;">Tambah Jabatan</h4>
      </div>

      <form action="<?= base_url('kepegawaian/jabatan/tambah') ?>" method="POST">
          <div class="modal-body">
                <div class="form-group">
                    <label for="input-jabatan">Jabatan</label>
                    <input type="text" class="form-control" name="jabatan" id="input-jabatan" required>
                </div>
          </div>
...
<div id="modal-edit-data" class="modal fade">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="<?= base_url('kepegawaian/jabatan/edit') ?>" method="POST">
          <div class="modal-body">
                <div class="form-group">
                    <label for="input-jabatan">Jabatan</label>
...
<script>
    ...
    function get_data() {
        ...
        $.ajax({
            url: '<?= base_url('kepegawaian/jabatan/get_data') ?>',
            method: 'GET',
            data: {search},
            dataType: 'json',
            ...
            success: function (res) {
                ...
                if(res.length > 0) {
                    ...
                    tr += `
                      <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-warning btn-sm" type="button" onclick="edit_data(${item.id_jabatan}, '${item.jabatan}')" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-pencil"></i></button>
                            <button class="btn btn-danger btn-sm" type="button" onclick="hapus_data(${item.id_jabatan})" data-toggle="tooltip" data-original-title="Close"><i class="fa fa-trash"></i></button>
                        </td>
                      </tr>
                    `
                  }
                  ...
                  tr = `
                    <tr>
                      <td colspan="${count_col}" class="text-center text-muted" ><span style="display: block; padding: 12px;">Data Jabatan Belum Ada, Tambahkan Data Jabatan</span></td>
                    </tr>
                  `
                }
    ...
    function hapus_data(id) {
        swal({
            ...
            closeOnConfirm: false
                ...
                $.ajax({
                url: '<?= base_url('kepegawaian/jabatan/hapus') ?>',
                method: 'GET',
                data: {id},
                dataType: 'json',
                success: function (res) {
