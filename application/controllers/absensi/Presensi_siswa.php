...
class Presensi_siswa extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('absensi/M_presensi_siswa', 'model');
    ...
    $this->load->view('absensi/presensi_siswa/index', $data);
	...
	public function export_excel()
	{
		...
		$pilih_siswa = $this->input->get('pilih_siswa');
    ...
    $get_data = $this->model->get_data($pilih_kelas, $pilih_siswa, $date_start_cek, $date_end_cek);
    ...
    $sheet->getStyle('G3')->applyFromArray($style_col);
		...
		foreach ($get_data as $key => $data) {
			...
			$sheet->setCellValue('B'.$x, $data['kelas']);
			$sheet->setCellValue('C'.$x, $data['nama_siswa']);
    ...
    $sheet->getDefaultRowDimension()->setRowHeight(-1);
  	...
  	$writer = new Xlsx($spreadsheet);
		if($date_start == '' && $date_end == '') {
			$currentDate = date('d-m-Y');
			$filename = 'laporan-rekap-absensi-siswa-'.$currentDate;
		} else {
			$filename = 'laporan-rekap-absensi-siswa-'.$date_start.'-sd-'.$date_end;
		}
    ...
    $data = $this->model->get_data($pilih_kelas, $pilih_siswa, $date_start_cek, $date_end_cek);
